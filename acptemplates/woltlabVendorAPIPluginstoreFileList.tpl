				...
				{foreach from=$objects item=pluginstoreFile}
					<tr class="jsPluginstoreRow" id="pluginstoreFile{@$pluginstoreFile->getObjectID()}">
						<td class="columnIcon">
							...
							<span class="icon icon16 fa-{if !$pluginstoreFile->isDisabled}square{else}check-square{/if}-o pointer jsTooltip jsToggleButton" data-object-id="{@$pluginstoreFile->getObjectID()}" title="{lang}wcf.global.button.{if $pluginstoreFile->isDisabled}enable{else}disable{/if}{/lang}"></span>
